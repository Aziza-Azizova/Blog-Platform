...
UserController.get(
    ...
    async (req, res, next) => {
        try {
            const profileData = await UserService.showProfile(req, res);
...
UserController.put(
    '/profile',
    validate(userProfileDtoSchema),
    authTokenMiddleware,
    async (req, res, next) => {
        try {
            const updatedProfileData = await UserService.updateProfile(req, res);
...
UserController.put(
    '/:id/role',
    validate(userRoleDtoSchema),
    authTokenMiddleware,
    async (req, res, next) => {
        try {
            const updatedUserRole = await UserService.updateUserRole(req, res);
